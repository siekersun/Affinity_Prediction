import h5py
import numpy as np
import pandas as pd
import torch
import json
import ast
from torch.utils.data import Dataset, DataLoader

def load_embeddings(file_bit_fp, file_count_fp, GPepT_emb_path, file_ChemBERTa):
    """Load desc / fp embeddings."""
    with h5py.File(file_bit_fp, "r") as f:
        seqs_smiles1 = f["seq"][:].astype(str)
        bit_fp = f["fp"][:]

    data = np.load(file_count_fp)
    count_fp=data['X']
    seqs_smiles2=data['seq_ids']
    data.close()

    with h5py.File(file_ChemBERTa, "r") as f:
        seqs_ChemBERTa = f["seq"][:].astype(str)
        Chem_emb = f["embed"][:]

    seq_to_idx = {s: i for i, s in enumerate(seqs_smiles1)}
    GPepT_emb = pd.read_csv(GPepT_emb_path)

    assert (seqs_smiles1 == seqs_smiles2).all()
    assert (seqs_smiles2 == seqs_ChemBERTa).all()

    NATURAL_AA = set('ACDEFGHIKLMNPQRSTVWY')
        
    GPepT_emb = {
        row['token']: np.array(ast.literal_eval(row['embedding']), dtype=float)
        for _, row in GPepT_emb.iterrows() if row['token'] in NATURAL_AA
    }

    return seq_to_idx, bit_fp, count_fp, Chem_emb, GPepT_emb

def load_split_data(csv_path, seq_to_idx, bit_fp, count_fp, Chem_emb, log_target=True, median_center=False):
    """Load a CSV split and return embeddings + labels."""
    df = pd.read_csv(csv_path)

    seq_ids = df["PepetideID"].tolist()
    y = df.iloc[:, 1].astype("float32").values.reshape(-1, 1)

    if log_target:
        y = np.log10(y)

    bit_fp_split = bit_fp[[seq_to_idx[s] for s in seq_ids]]
    count_fp_split = count_fp[[seq_to_idx[s] for s in seq_ids]]
    Chem_emb_split = Chem_emb[[seq_to_idx[s] for s in seq_ids]]

    return seq_ids, y, bit_fp_split, count_fp_split, Chem_emb_split


class TensorDataset(Dataset):
    def __init__(self, seq, y, bit_fp, count_fp, Chem_emb, GPepT_emb):


        self.mon_emb = GPepT_emb
        self.seq = seq
        self.y = torch.tensor(y, dtype=torch.float32)
        self.bit_fp = torch.from_numpy(bit_fp).float()
        self.count_fp = torch.from_numpy(count_fp).float()
        self.Chem_emb = torch.from_numpy(Chem_emb).float()

    def __len__(self):
        return len(self.seq)

    def __getitem__(self, idx):
        seq_i = self.seq[idx]
        bit_fp = self.bit_fp[idx]
        count_fp = self.count_fp[idx]
        Chem_emb_fp = self.Chem_emb[idx]
        y = self.y[idx]
        mon_emb = np.stack([self.mon_emb[aa] for aa in seq_i], axis=0)
        mon_emb = torch.from_numpy(mon_emb).float()
        return mon_emb, bit_fp, count_fp, Chem_emb_fp, y

class DynamicMegaBatchLoader:
    def __init__(self, train_ds, valid_ds, test_ds, n_batches=1181, collate_fn=None):
        self.n_train = len(train_ds)
        self.n_valid = len(valid_ds)
        self.n_test  = len(test_ds)

        self.n_batches = n_batches

        # 每个 split 的 batch size（向下取整）
        self.train_bs = max(0, self.n_train // self.n_batches)
        self.valid_bs = max(0, self.n_valid // self.n_batches)
        self.test_bs  = max(0, self.n_test  // self.n_batches)

        if (self.train_bs == 0) or (self.valid_bs == 0) or (self.test_bs == 0):
            raise ValueError(
                f"One of split batch sizes is 0 (train_bs={self.train_bs}, "
                f"valid_bs={self.valid_bs}, test_bs={self.test_bs}). "
                "Reduce n_batches or provide larger datasets."
            )

        self.train_ds = train_ds
        self.valid_ds = valid_ds
        self.test_ds  = test_ds

        if collate_fn is None:
            raise ValueError("collate_fn must be provided")
        self.collate_fn = collate_fn

    def __len__(self):
        return self.n_batches

    def __iter__(self):
        train_idx = torch.randperm(self.n_train)
        valid_idx = torch.randperm(self.n_valid)
        test_idx  = torch.randperm(self.n_test)

        for i in range(self.n_batches):
            t_start = i * self.train_bs
            v_start = i * self.valid_bs
            te_start = i * self.test_bs

            t_idx = train_idx[t_start : t_start + self.train_bs]
            v_idx = valid_idx[v_start : v_start + self.valid_bs]
            te_idx = test_idx[te_start : te_start + self.test_bs]

            # list of tuples: [(x,sf,y), ...]
            train_batch = [self.train_ds[j] for j in t_idx]
            valid_batch = [self.valid_ds[j] for j in v_idx]
            test_batch  = [self.test_ds[j]  for j in te_idx]

            # ===== 推荐：一次性合并后调用 collate_fn =====
            combined = train_batch + valid_batch + test_batch  # 保证顺序：train, valid, test
            mon_emb, bit_fp, count_fp, Chem_emb_fp, mask, y = self.collate_fn(combined)

            # split_id：与 combined 顺序对应
            split_id = torch.cat([
                torch.zeros(self.train_bs, dtype=torch.long),
                torch.ones(self.valid_bs, dtype=torch.long),
                torch.full((self.test_bs,), 2, dtype=torch.long)
            ])

            yield mon_emb, bit_fp, count_fp, Chem_emb_fp, mask, y, split_id


def collate_fn(batch):
    mon_emb, bit_fp, count_fp, Chem_emb_fp, y = zip(*batch) 
    lengths = [x.size(0) for x in mon_emb]
    max_len = max(lengths)

    # Padding
    padded = []
    for x in mon_emb:
        pad_len = max_len - x.size(0)
        if pad_len > 0:
            x = torch.cat([x, torch.zeros(pad_len, x.size(1))], dim=0)
        padded.append(x)

    mon_emb = torch.stack(padded)
    bit_fp = torch.stack(bit_fp)
    count_fp = torch.stack(count_fp)
    Chem_emb_fp = torch.stack(Chem_emb_fp)
    y = torch.stack(y)
    mask = torch.tensor([[1]*l + [0]*(max_len-l) for l in lengths])
    
    # 仅返回 peptide embedding, mask, 和 label
    return mon_emb, bit_fp, count_fp, Chem_emb_fp, mask, y 

def get_dataloaders(
    train_csv,
    valid_csv,
    test_csv,
    file_bit_fp,
    file_count_fp,
    GPepT_emb_path,
    file_ChemBERTa,
    mix= False
):
    """Main function: return three DataLoaders."""

    # load embeddings
    seq_to_idx, bit_fp, count_fp, Chem_emb, GPepT_emb = load_embeddings(file_bit_fp, file_count_fp, GPepT_emb_path, file_ChemBERTa)

    # load each split
    seq_train, y_train, bit_fp_train, count_fp_train, Chem_emb_train = load_split_data(
        train_csv, seq_to_idx, bit_fp, count_fp, Chem_emb,
    )
    seq_valid, y_valid, bit_fp_valid, count_fp_valid, Chem_emb_valid = load_split_data(
        valid_csv, seq_to_idx, bit_fp, count_fp, Chem_emb,
    )
    seq_test, y_test, bit_fp_test, count_fp_test, Chem_emb_test = load_split_data(
        test_csv, seq_to_idx, bit_fp, count_fp, Chem_emb,
    )

    train_dataset = TensorDataset(seq_train, y_train, bit_fp_train, count_fp_train, Chem_emb_train, GPepT_emb)
    valid_dataset = TensorDataset(seq_valid, y_valid, bit_fp_valid, count_fp_valid, Chem_emb_valid, GPepT_emb)
    test_dataset  = TensorDataset(seq_test, y_test, bit_fp_test, count_fp_test, Chem_emb_test, GPepT_emb)
    
    if mix:
        mega_loader = DynamicMegaBatchLoader(train_dataset, valid_dataset, test_dataset, n_batches=1181,collate_fn=collate_fn)

        return mega_loader
    else:
        BATCH = 2837
        train_loader = DataLoader(train_dataset, batch_size=BATCH, shuffle=True, collate_fn=collate_fn, drop_last=True)
        valid_loader = DataLoader(valid_dataset, batch_size=BATCH, shuffle=False, collate_fn=collate_fn, drop_last=False)
        test_loader  = DataLoader(test_dataset,  batch_size=BATCH, shuffle=False, collate_fn=collate_fn, drop_last=False)
        
        return train_loader, valid_loader, test_loader
