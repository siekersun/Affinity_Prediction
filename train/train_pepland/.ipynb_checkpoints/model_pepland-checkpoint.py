...
class FCLayer(nn.Module):
    def __init__(self, in_dim, out_dim, max_dim=512, dp=0.5):
...
class TransformerRegressor(nn.Module):
    def __init__(self, emb_dim, hidden_dim=256, nhead=8, nfc=15, nlayers=1, dp=0.5):
        super().__init__()
    ...
    def forward(self, x, smiles_x ): 
        ...
        # x_emb = self.res_x(x)+x_emb
        # smiles_emb = self.res_smiles(smiles_x)+smiles_emb
        all_x = torch.concat([x_emb, smiles_emb, x_emb*smiles_emb], dim=-1)
        # all_x = self.res_fc1(torch.concat([smiles_x, x], -1))+all_x
        all_res = all_x
