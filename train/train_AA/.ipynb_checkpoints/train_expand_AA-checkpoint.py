...
save_path = 'train_AA/save/best_expand_AAmodel.pth'
patience = 5
counter = 0
...
for epoch in range(EPOCH):
    ...
    for x, smiles_f, mask, y, split_id in tqdm(loader, desc="Training and Validation"):

        x, smiles_f, mask, y, split_id = x.to(device), smiles_f.to(device), mask.to(device), y.to(device), split_id.to(device)

        # ------------------------------------------------------
        # 1) 前向传播（整个 mega-batch 只跑一次）
        # ------------------------------------------------------
        if e_x is None:
    ...
    if avg_val_loss < best_val_loss:
        ...
        np.save('train_AA/save/best_e_x_AA_pand.npy', e_x.detach().cpu().numpy())
...
def evaluate_mega(loader, model, criterion, device, e_x_path=None):
    ...
    De = torch.zeros(1024).to(device)
    if e_x_path is not None:
        e_x = torch.from_numpy(np.load(e_x_path)).to(device)
    else:
        e_x = None

    with torch.no_grad():
        for x, smiles_f, mask, y, split_id in tqdm(loader, desc="Evaluation"):
            x = x.to(device)
            ...
            # forward 只跑一次
            pred, e_x, De = model(x, smiles_f, mask, e_x, De, 0)
